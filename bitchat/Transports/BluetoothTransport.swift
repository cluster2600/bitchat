...
class BluetoothTransport: NSObject, TransportProtocol {
    var transportType: TransportType = .bluetooth
    ...
    weak var transportDelegate: TransportDelegate?
    ...
    weak var delegate: BitchatDelegate? {
        didSet {
            // This is for the old delegate system, we will phase this out.
            // For now, we can use it to keep the app working.
        }
    }
    ...
    func startAdvertising() {
        ...
        advertisementData = [
            CBAdvertisementDataServiceUUIDsKey: [BluetoothTransport.serviceUUID],
    ...
    private func setupPeripheral() {
        let characteristic = CBMutableCharacteristic(
            type: BluetoothTransport.characteristicUUID,
            properties: [.read, .write, .writeWithoutResponse, .notify],
            value: nil,
            permissions: [.readable, .writeable]
        )
        
        let service = CBMutableService(type: BluetoothTransport.serviceUUID, primary: true)
    ...
    private func sendAnnouncementToPeer(_ peerID: String) {
        ...
        if let data = packet.toBinaryData() {
            ...
            if let peripheral = connectedPeripherals[peerID],
               peripheral.state == .connected,
               let characteristic = peripheral.services?.first(where: { $0.uuid == BluetoothTransport.serviceUUID })?.characteristics?.first(where: { $0.uuid == BluetoothTransport.characteristicUUID }) {
...
extension BluetoothTransport: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if let name = peripheral.name, name.count == 8 {
            ...
            transportDelegate?.didDiscoverPeer(peerID)
    ...
    func send(_ packet: Data, to peer: String?) {
        if let peer = peer, let peripheral = connectedPeripherals[peer], let characteristic = peripheralCharacteristics[peripheral] {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if let peerID = foundPeerID {
            ...
            transportDelegate?.didLosePeer(peerID)
            // Peer disconnected
        ...
        if centralManager?.state == .poweredOn {
            ...
            centralManager?.stopScan()
            centralManager?.scanForPeripherals(withServices: [BluetoothTransport.serviceUUID], options: [CBCentralManagerScanOptionAllowDuplicatesKey: false])
        }
    }
}
...
extension BluetoothTransport: CBPeripheralDelegate {
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in services {
            peripheral.discoverCharacteristics([BluetoothTransport.characteristicUUID], for: service)
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for characteristic in characteristics {
            if characteristic.uuid == BluetoothTransport.characteristicUUID {
                ...
                if let vm = self.delegate as? ChatViewModel {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self, weak peripheral] in
                        guard let self = self,
                              ...
                              peripheral.state == .connected,
                              let characteristic = peripheral.services?.first(where: { $0.uuid == BluetoothTransport.serviceUUID })?.characteristics?.first(where: { $0.uuid == BluetoothTransport.characteristicUUID }) else { return }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        transportDelegate?.didReceiveData(data, from: peerID)
